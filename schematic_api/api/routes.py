...
class ManifestGeneration():
        ...
        if self.output_format == "excel":
            ...
            for i, dt in enumerate(data_type_lst):
                ...
                if dataset_id:
                    ...
                    output = self.create_single_manifest(access_token=access_token, single_data_type=dt, single_dataset_id=dataset_id[i], title=t)
                else:
                    # get new manifests in google sheet or data frame format
                    output = self.create_single_manifest(access_token=access_token, single_data_type=dt, title=t)
    ...
    def get_manifests_route(schema_url: str, data_type:List[str], use_annotations: Optional[str]=None, dataset_id: Optional[List[str]]=None, title: Optional[str]=None, asset_view: Optional[str]=None, output_format: Optional[str]="google_sheet") -> str|pd.DataFrame|BinaryIO:
        ...
        all_outputs = mg.generate_manifest_and_collect_outputs(access_token=access_token, data_type_lst=components, dataset_id=dataset_id)
