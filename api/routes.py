...
def get_manifest_route(
    schema_url, title, oauth, use_annotations, dataset_ids=None, asset_view=None
):
    # call config_handler()
...
def submit_manifest_route(
    schema_url, asset_view=None, manifest_record_type=None, json_str=None
):
    # call config_handler()
    config_handler(asset_view=asset_view)
...
def get_files_storage_dataset(
    input_token, asset_view, dataset_id, full_path, file_names=None
):
    # call config handler
    config_handler(asset_view=asset_view)
...
def get_viz_attributes_explorer(schema_url):
    ...
    attributes_csv = AttributesExplorer(temp_path_to_jsonld).parse_attributes(
        save_file=False
    )
...
def download_manifest(
    input_token, dataset_id, asset_view, as_json, new_manifest_name=""
):
    # call config handler
    config_handler(asset_view=asset_view)
...
def get_property_label_from_display_name(
    schema_url: str, display_name: str, strict_camel_case: bool = False
) -> str:
    ...
    label = explorer.get_property_label_from_display_name(
        display_name, strict_camel_case
    )
    return label
