...
class TestManifestValidation:
    def test_valid_manifest(self, helpers, metadataModel):
        ...
        errors, warnings = metadataModel.validateModelManifest(
            manifestPath=manifestPath, rootNode=rootNode
        )
    ...
    def test_invalid_manifest(self, helpers, sg, metadataModel):
        ...
        assert (
            GenerateError.generate_type_error(
                val_rule="num", row_num=3, attribute_name="Check Num", invalid_entry="c"
            )
            in errors
        )
    ...
    def test_rule_combinations(
        self, helpers, sg, base_rule, second_rule, metadataModel
    ):
        ...
        for attribute in sg.se.schema[
            "@graph"
        ]:  # Doing it in a loop becasue of sg.se.edit_class design
            if "sms:validationRules" in attribute and attribute["sms:validationRules"]:
                if base_rule in attribute["sms:validationRules"] or re.match(
                    rule_regex, attribute["sms:validationRules"][0]
                ):
        ...
        validateManifest = ValidateManifest(
            ...
            manifestPath=manifestPath,
            sg=sg,
            jsonSchema=sg.get_json_schema_requirements(
