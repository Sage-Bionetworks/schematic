...
class TestManifestValidation:
    ...
    def test_component_validations(self, helpers, manifest_path):
        ...
        data_model_js = DataModelJSONSchema(jsonld_path=helpers.get_data_path('example_new_vrs.model.csv'), graph=dmge.graph)

        json_schema = data_model_js.get_json_validation_schema(source_node=root_node, schema_name=root_node + "_validation")
        ...
        _, vmr_errors, vmr_warnings = validateManifest.validate_manifest_rules(
            manifest=manifest, dmge=dmge, restrict_rules=False, project_scope=None,
            )

        if root_node == 'Biospecimen':
            assert vmr_errors and vmr_errors[0][0] == ['2', '3'] and vmr_errors[0][-1] == ['123']
            assert vmr_warnings == []
        elif root_node == 'Patient':
            assert vmr_errors == []
            assert vmr_warnings and vmr_warnings[0][0] == ['2', '3'] and vmr_warnings[0][-1] == ['123']
