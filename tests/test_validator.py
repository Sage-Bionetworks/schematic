...
class TestDataModelValidator:
    def test_check_blacklisted_characters(self, helpers):
        ...
        graph_data_model = graph_data_model_func(helpers, data_model_name='validator_test.model.csv')
        ...
        # Expected Error
        expected_error = ['Node: Patient) contains a blacklisted character(s): ), they will be striped if used in Synapse annotations.',
    ...
    def test_check_reserved_names(self, helpers):
        ...
        # Expected Error
        expected_error = ['Your data model entry name: EntityId overlaps with the reserved name: entityId. Please change this name in your data model.']
        assert expected_error == validator_errors
    ...
    def test_check_graph_has_required_node_fields(self, helpers):
        ...
        # Expected Error
        expected_error = ['For entry: Cancer, the required field label is missing in the data model graph, please double check your model and generate the graph again.']
        assert expected_error == validator_errors
    ...
    def test_dag(self, helpers):
        ...
        expected_errors = ['Schematic requires models be a directed acyclic graph (DAG). Please inspect your model.']
