...
def get_error(
    validation_rules: list, attribute_name: str, error_type: str, input_filetype: str
) -> List[str]:
    """
...
def validate_single_rule(validation_rules, errors, attribute, input_filetype):
    ...
    if validation_rule_with_args[0] not in validation_types.keys():
        ...
        if num_args:
            argument_allowed, num_allowed = validation_types[
                validation_rule_with_args[0]
            ]["arguments"]
...
def validate_schema_rules(validation_rules, attribute, input_filetype):
            ...
            if second_type == "regex":
                errors.extend(
                    validate_single_rule(
                        validation_rules[1], errors, attribute, input_filetype
                    )
                )
    ...
    # throw an error if they have.
        ...
        errors.extend(
            validate_single_rule(validation_rules[0], errors, attribute, input_filetype)
        )
