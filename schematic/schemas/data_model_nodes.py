...
class DataModelNodes():
    ...
    def run_rel_functions(self, rel_func:callable, node_display_name:str='', key:str='', attr_relationships={}, csv_header='', entry_type=''):
        ...
        if rel_func == get_attribute_display_name_from_label:
            return get_attribute_display_name_from_label(node_display_name, attr_relationships)
            ...
            return get_label_from_display_name(display_name =node_display_name, entry_type=entry_type)
    ...
    def generate_node_dict(self, node_display_name: str, attr_rel_dict: dict) -> dict:
        ...
        for key, csv_header in self.value_relationships.items():
            ...
            if csv_header in attr_relationships.keys():
                ...
                if 'standard' in rel_node_dict.keys() and isfunction(rel_node_dict['standard']):
                    # Add to node_dict The value comes from the standard function call. 
                    node_dict.update({rel_key: self.run_rel_functions(rel_node_dict['standard'], node_display_name=node_display_name, key=key, attr_relationships=attr_relationships, csv_header=csv_header, entry_type=entry_type)})
                else:
            ...
            # else, add default values
                ...
                if 'default' in rel_node_dict.keys() and isfunction(rel_node_dict['default']):
                    node_dict.update({rel_key: self.run_rel_functions(rel_node_dict['default'], node_display_name=node_display_name, key=key, attr_relationships=attr_relationships, csv_header=csv_header, entry_type=entry_type)})
                else:
