...
_VALIDATION_RULES = [
    ValidationRule(ValidationRuleName.LIST, None, False),
    ValidationRule(
...
def filter_unused_rules(rules: list[str]) -> list[str]:
    ...
    Returns:
        list[str]: A filtered list of validation rules
    """
    return [
        rule
        for rule in rules
...
def check_for_rule_conflicts(input_rules: list[str]) -> None:
    ...
    Arguments:
        input_rules: A list of validation rules

    Raises:
        ValueError: If a rule is in conflict with any other rule
    """
    rule_names = [input_rule.split(" ")[0] for input_rule in input_rules]
    rules: list[ValidationRule] = _get_rules_by_name(rule_names)
    for rule in rules:
        if rule.incompatible_rules:
...
def get_rule_from_rule_list(
    ...
    """
    rules = [rule for rule in rules if rule.startswith(rule_name.value)]
    if len(rules) > 1:
        raise ValueError("Found duplicates of rule in rules: ", rules)
    if len(rules) == 0:
        return None
    return rules[0]


def get_js_type_from_rule_list(rules: list[str]) -> Optional[str]:
...
def _get_parameters_from_input_rule(input_rule: str) -> Optional[dict[str, str]]:
    ...
    Args:
        input_rule: A validation rule input
    ...
    Returns:
        ...
        Else None
    """
    rule_name = input_rule.split(" ")[0]
    rule_values = input_rule.split(" ")[1:]
