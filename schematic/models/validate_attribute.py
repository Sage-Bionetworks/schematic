...
class GenerateError:
    ...
    def generate_content_error(
        ...
        if val_rule.startswith("recommended"):
            cross_error_str = f"Column {attribute_name} is recommended but empty."
            ...
            cross_error_str = f"Column {attribute_name} has the duplicate value(s) {set(error_val)} in rows: {row_num}."

        elif val_rule.startswith("protectAges"):
            cross_error_str = f"Column {attribute_name} contains ages that should be censored in rows: {row_num}."

        elif val_rule.startswith("inRange"):
            cross_error_str = f"{attribute_name} values in rows {row_num} are out of the specified range."
...
class ValidateAttribute(object):
    ...
    def get_target_manifests(target_component):
        ...
        for project in projects:
            ...
            for target_dataset in target_datasets:
                ...
                if (
                    target_component.lower()
                    == target_dataset[-1][0].replace(" ", "").lower()
                ):
    ...
    def cross_validation(
        ...
        (
            ...
            target_manifest_IDs,
            target_dataset_IDs,
        ) = ValidateAttribute.get_target_manifests(target_component)
        ...
        if val_rule.__contains__("matchAtLeastOne") and len(present_manifest_log) < 1:
            for missing_ID in missing_manifest_log:
                ...
                for row, value in zip(
                    missing_dict.keys(), missing_dict
                ):  # wrong dict used, cause of not all errors being raised
                    row = row + 2
                    errors.append(
            ...
            val_rule.__contains__("matchExactlyOne") and len(present_manifest_log) != 1
        ):
            errors.append(
                GenerateError.generate_cross_error(
                    val_rule=val_rule,
